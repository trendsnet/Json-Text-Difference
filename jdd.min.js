...
val1.forEach(function(arrayVal,index){if(val2.length<=index){}else{config1.currentPath.push('/['+index+']');config2.currentPath.push('/['+index+']');if(getType(val2)==='array'){diffVal(val1[index],config1,val2[index],config2)}
...
return{path1:pathObj1,path2:pathObj2,type:type,msg:msg}};function forEach(array,callback,scope){for(let idx=0;idx<array.length;idx++){callback.call(scope,array[idx],idx,array)}}
function diffColor(diffs,blockLeftId,blockRightId){for(i=0;i<diffs.length;i++){let linesLeft=$(`#${blockLeftId}`).text().split('\n');let textLeft=$(`#${blockLeftId}`).text();for(let j=0;j<linesLeft.length;j++){if(diffs[i].path1.line==j+1){$(`#${blockLeftId}`).text(textLeft.replace(linesLeft[j],`<span style="color:#c0392b;">${linesLeft[j]}</span>`))}}
let linesRight=$(`#${blockRightId}`).text().split('\n');let textRight=$(`#${blockRightId}`).text();for(let j=0;j<linesRight.length;j++){if(diffs[i].path2.line==j+1){$(`#${blockRightId}`).text(textRight.replace(linesRight[j],`<span style="color:#c0392b;">${linesRight[j]}</span>`))}}}
