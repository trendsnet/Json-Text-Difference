(function($){$.fn.jdd=function(blockLeftId,blockRightId){diffs=[];let left=$(`#${blockLeftId}`).text().length!=0?JSON.parse($(`#${blockLeftId}`).text()):JSON.parse('{}');let right=$(`#${blockRightId}`).text().length!=0?JSON.parse($(`#${blockRightId}`).text()):JSON.parse('{}');let config=createConfig();formatAndDecorate(config,left);$(`#${blockLeftId}`).text(config.out);let config2=createConfig();formatAndDecorate(config2,right);$(`#${blockRightId}`).text(config2.out);config.currentPath=[];config2.currentPath=[];diffVal(left,config,right,config2);diffColor(diffs,blockLeftId,blockRightId)};const LEFT='left';const RIGHT='right';const EQUALITY='eq';const TYPE='type';const MISSING='missing';let diffs=[];const requestCount=0;function createConfig(){return{out:'',indent:-1,currentPath:[],paths:[],line:1}};function formatAndDecorate(config,data){if(getType(data)==='array'){formatAndDecorateArray(config,data);return}
